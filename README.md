...
Then run `pnpm install` to install the dependencies and `pnpm build` to build the project.

Start PostgreSQL database: `docker-compose -f docker-compose-db.yml up`

To start the agent, run `pnpm run dev --characters="characters/pixels.character.json"`.
